...
fn floatingWindow(position: *rl.Vector2, size: *rl.Vector2, minimized: *bool, moving: *bool, resizing: *bool, draw_content: DrawContentFn, content_size: rl.Vector2, scroll: *rl.Vector2, title: []const u8) void {
    ...
    if (minimized.*) {
        ...
        if (rg.button(rl.Rectangle{ .x = position.x + size.x - WINDOW_CLOSE_BUTTON_SIZE - CLOSE_TITLE_SIZE_DELTA_HALF, .y = position.y + CLOSE_TITLE_SIZE_DELTA_HALF, .width = WINDOW_CLOSE_BUTTON_SIZE, .height = WINDOW_CLOSE_BUTTON_SIZE }, "#120#")) {
            minimized.* = false;
        }
    } else {
        ...
        _ = rg.scrollPanel(rl.Rectangle{ .x = position.x, .y = position.y + WINDOW_STATUS_BAR_HEIGHT, .width = size.x, .height = size.y - WINDOW_STATUS_BAR_HEIGHT }, null, rl.Rectangle{ .x = position.x, .y = position.y, .width = content_size.x, .height = content_size.y }, scroll, &scissor);
